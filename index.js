...
const json = require('styledexp.json')
/* *************************************** Server Startup *************************************** */
const app = Express();
...
function handleMessage(socket, username, strMessage) {
  ...
  switch (message.kind) {
    case 'chat':
      broadcast(Message.CHAT(username, message.data));
      break;
    case 'userlist':
      socket.send(Message.USER_LIST(username));
      break;
    case 'whoami':
      socket.send(Message.WHOAMI(username));
      break;
    case 'direct':
      ...
      break;
    default:
      socket.send(Message.KIND_ERROR(username));
  }
}
